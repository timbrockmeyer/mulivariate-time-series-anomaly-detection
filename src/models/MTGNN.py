...
class MTGNNModel(torch.nn.Module):
    def __init__(self, config):
        ...
        # layer definitions
        self.embedding = SingleEmbedding(
            self.num_nodes, 
            self.embed_dim, 
            topk=self.topk
        ).to(self.device)
    ...
    def forward(self, window):
        x = window.view(-1, 1, self.num_nodes, self.lags) # (batch, 1, num_nodes, lags)    
        pred = self.gnn(x) # (batch, out_channels, num_nodes, 1)
        pred = pred.squeeze(-1).permute(0,2,1).contiguous().view(-1, self.horizon) # batch stacked
